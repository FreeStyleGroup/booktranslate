...
export function ExportPanel({
  ...
  async function download(): Promise<void> {
    ...
    const name =
      attachmentName(response.headers.get("content-disposition")) ?? `перевод.${format}`;
  ...
  return (
    <section className="tile ex" id="export">
      ...
      {draft && (
        <p className="wk-note ex-draft" role="status">
          ...
          <span>
            ...
            {plural(untranslated, "сегмент уйдёт", "сегмента уйдут", "сегментов уйдут")} в
            файл исходным текстом. Это черновик — показать середину работы или отдать главу
            на вычитку, а не сдавать заказчику.
          </span>
        </p>
      )}
      ...
      <p className="tile__note wk-seg__foot">
        Оформление внутри абзаца — полужирное слово, ссылка на трёх словах — в
        переводе не сохраняется: сегмент хранит текст, а не разметку. Всё
        остальное на месте: стиль абзаца, картинки, сноски, таблицы и
        оглавление.
      </p>
    </section>
  );
